...
class Puzzle8:
    ...
    def _genRandomItems(self):          #随机产生初始8数码
        items = Puzzle8.ITEMS[:]
    ...
    def _calcInversions(self):
        def merge_sort(items):
            if len(items) <= 1:
                return items
            pos = len(items) // 2       # '//' 为整除除法，结果为整数
            half1 = items[:pos]         #分割items,前面一半为items[0] -> pos ,后面一半为 pos -> items[-1]
            half2 = items[pos:]
            left = merge_sort(half1)        #递归
        ...
        items = [int(self[row, col]) \
                     for row in range(Puzzle8.HEIGHT) \
                         for col in range(Puzzle8.WIDTH) \
                             if self[row, col] != ' ']
        merge_sort(items)
        print("inversion = ", self.__inversions)
        ...
        for row in range(Puzzle8.HEIGHT):
            for col in range(Puzzle8.WIDTH):
                ...
                if item != ' ':
                    ...
                    print("row = {0},   row1 ={1},  ".format(row,row1))
                    col1 = index % Puzzle8.WIDTH
                    print("col = {0},   col1 = {1}, ".format(col,col1))
                    heuristics += abs(row1-row) + abs(col1-col)
                    print("index = {0}, item = {1},     h(item) = {2} ".format(index,item,heuristics))
